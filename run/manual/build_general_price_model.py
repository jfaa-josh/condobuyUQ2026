...
from condobuyuq2026.utils.manual_utils import (
    fetch_bls_series,  # noqa: F401 -- used by the commented-out fetch block below
    filter_monthly,  # noqa: F401 -- used by the commented-out fetch block below
    load_raw_csv,
    model_fit_path,
    model_plots_dir,
    raw_csv_path,
    save_raw_csv,  # noqa: F401 -- used by the commented-out fetch block below
    to_monthly_series,
)
...
if __name__ == "__main__":
    # Uncomment to (re-)pull from BLS and overwrite data/raw/<MODEL_NAME>.csv. Left commented out
    # so re-running this script (e.g. while iterating on the fit below) doesn't burn API quota.
    # Everything past this block always reads back from data/raw/, never from data_points/
    # monthly_points directly -- so it works the same whether this ran this time or not.
